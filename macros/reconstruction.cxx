...
const double kNoisefrac = 0.02;      // ratio for noise/multiplicity points
...
void reconstruction()
{
  ...
  if (mul == "No")
    ...
    limit = kLimit1; // if not uniform multi we stop before
  ...
  for (int ev = 0; ev < inputTree->GetEntries(); ev++)
  {
    ...
    for (int i = 0; i < nlines1; i++)
    {
      ...
      if (TMath::Abs(i1->GetZ()) < L / 2)
      { // check that after smearing the hit point still lies in T1
        x1 = i1->GetX();
        y1 = i1->GetY();
        z1 = i1->GetZ();
    ...
    for (int i = 0; i < nlines2; i++)
    {
      ...
      if (TMath::Abs(i2->GetZ()) < L / 2)
      { // check that after smearing the hit point still lies in T2
        x2 = i2->GetX();
        y2 = i2->GetY();
        z2 = i2->GetZ();
    ...
    if (ev % kVerbosityReco == 0)
      cout << "smearing of hits done" << endl;
    if (ev % kVerbosityReco == 0)
      cout << "Hit outside T1: " << out1 << "  Hit outside T2: " << out2 << endl;
    ...
    for (int i = 0; i < Nrec1; i++)
    {
      for (int j = 0; j < Nrec2; j++)
      {
        ...
        if (TMath::Abs(i2->GetPhi() - i1->GetPhi()) < kPhiMax)
        { // Creation of Tracklet for points in phimax range
          x1 = i1->GetX();
          x2 = i2->GetX();
          y1 = i1->GetY();
          y2 = i2->GetY();
          ...
          { // condition for the intersection method to have worked
    ...
    if (ev % kVerbosityReco == 0)
      cout << "the tracklets are generated and the histo is filled" << endl;
    if (zIntersecHisto->GetEntries() != 0)
    {                                                      // check that the histo is not empty
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        { // range choice is taken from an observation of the histo
      ...
      if (denom > 0)
      {
        ...
        if (denom > 1)
          zrecrms = TMath::Sqrt(deviation / (denom - 1)); // standard deviation
        else
        {
          zrecrms = 0.05;
        } // 0.05 is chosen as error on an average of a single value
        if (ev % kVerbosityReco == 0)
    ...
    {
      new TCanvas;
      zIntersecHisto->DrawCopy();
    }
    if (verlec->GetZ() < 1 * sigmaZ && verlec->GetM() < multiBin[limit])
      tot1multi->Fill(verlec->GetM());
    if (verlec->GetZ() < 3 * sigmaZ && verlec->GetM() < multiBin[limit])
      tot3multi->Fill(verlec->GetM());
...
void analysis(const vector<double> *const vzTruep, const vector<double> *const vzrecp, const vector<int> *const vzmultip, const vector<double> *const vzrecrmsp, const double *const multiBin, const int arrayLenghtMulti, TH1D *tot1multi, TH1D *tot3multi, const int limit, const double sigmaZ, const double *const zbin, const int arrayLenghtZ, TH1D *totz, const int sizetrue)
{
  ...
  for (int i = 0; i < sizetrue; i++)
  {
    ...
    { // so that we fill just with good reconstructions and not with the cases where we took the wrong max
  ...
  // histograms well reconstructed events efficiency vs multiplicity
  TH1D *histo_ok[2];
  ...
  for (int i = 0; i < arrayLenghtZ; i++)
  {
    ...
    for (int j = 0; j < sizetrue; j++)
    {
      if (TMath::Abs(vzTrue[j] - vzrec[j]) < 3 * vzrecrms[j] && vzTrue[j] > zbin[i] && vzTrue[j] <= zbin[i + 1])
      {
        histo_residuizTrue[i]->Fill(vzrec[j] - vzTrue[j]);
      }
    }
